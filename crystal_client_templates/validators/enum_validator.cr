module OpenApi
  class EnumValidator
    ...
    def self.valid?(values : Array(T)?, allowable_values : StaticArray(T, N), allow_nil : Bool = true) : Bool forall T
      return true if allow_nil && values.nil?

      !values.nil? && values.not_nil!.all? { |value| allowable_values.includes?(value) }
    end

    def self.valid?(hash : Hash(U, T)?, allowable_values : StaticArray(T, N), allow_nil : Bool = true) : Bool forall T, U
